...
única exceção, e só no slide de fechamento ou CTA. Ver `identidade-visual.md`.
...
**Editar `visual/modelo/carrossel.html` à mão.** O arquivo é GERADO. Uma edição
...
**Editar `visual/marca/logo-base64.txt` à mão.** Também é gerado, a partir do PNG
...
**Versionar `publicacao/`.** É artefato reproduzível a partir de `conteudos/` +
`visual/`. `visual/exemplos/` é diferente: é referência visual validada, não
